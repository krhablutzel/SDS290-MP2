...
ggplot(cleaned_data, aes(x = trial, 
  ...
  geom_boxplot() +
  facet_wrap(~animal,
             labeller = labeller(animal = c("cat" = "Cat Photo Presented",
                                            "hedgehog" = "Hedgehog Photo Presented"))) +
  labs(title = "Happiness by Animal Presented, Time \nMeasurement, and Animal Preference", 
       x = "Time Measurement", 
       y = "Level of Happiness",
       color = "Animal Preference") +
  scale_x_discrete(labels = c('Before',
...
stats <- favstats(happiness ~ trial + animal_preference, data = cleaned_data)[,c(1,7,9)]
# diff of means
diff <- stats[2,2] - stats[4,2]
# use counts in leverage
n1 <- stats[2, 3]
